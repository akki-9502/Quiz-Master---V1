...
from quiz_master_v1.controllers.config import config
...
app = Flask(__name__,template_folder='templates',static_folder='static')#provide the path of the templates folder
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        if admin_user and admin_user.password == password:
            ...
            return redirect(url_for('adminhome'))  # Redirect to admin dashboard
        else:
...
def adminhome():
    ...
    return render_template('adminhome.html',subjects=subjects,chapters=chapters)
@app.route('/userhome')
def userhome():
...
def add_chapter():
    ...
    if request.method =='POST':
        ...
        new_chapter =Chapters(chapter_name=name, description=des, subject_id=subject_name)
...
def quiz():
    ...
    chapters_with_quizzes = Chapters.query.join(QuizDetails, Chapters.chaoter_id == QuizDetails.chapter_id).all()

    # Fetch all quizzes
    quizzes = QuizDetails.query.all()

    return render_template('quiz.html', chapters=chapters_with_quizzes, quizzes=quizzes)
...
def add_quiz():
    ...
    if request.method=='POST':
        ...
        new_quiz=QuizDetails(chapter_id=name,quiz_date=date,duration=duration)
...
def questions():
    ...
    return render_template('question.html', chapters=chapters, quizzes=quizzes, questions=questions)
...
def chapters():
    if request.method=='GET':
        ...
        return render_template('chapters.html',chapters=chapter,subjects=subject)
...
def edit_chapter(chapter_id):
    chapter = Chapters.query.get_or_404(chapter_id) #get the chapter, or return 404 if not found
    ...
    return render_template('editchapters.html', chapter=chapter, chapter_id=chapter_id)
...
def livequiz():
    ...
    attempted = get_attempted_quizzes(user_id)  # Your function to fetch attempted quizzes
...
def start_quiz(quiz_id):
    ...
    quiz = QuizDetails.query.filter(
        QuizDetails.quizid == quiz_id,
        QuizDetails.quiz_date >= date.today()
    ).first_or_404()
...
def submit_quiz(quiz_id):
    ...
    existing_attempt = db.session.execute(
        text("SELECT 1 FROM quizresponse WHERE quizid = :quiz_id AND uid = :uid"),
        {"quiz_id": quiz_id, "uid": user_id}
    ).fetchone()
    ...
    for question in questions:
        ...
        # Save all responses (even incorrect)
        response = QuizResponse(
            quizid=quiz_id,
            uid=user_id,
            questionid=question.qid,
            optionid=user_answer
        )
    ...
    db.session.add(Scores(
        ...
        time_stamp=datetime.now()  # Use datetime.now() consistently
    ))
    db.session.commit()
...
def quiz_results(quiz_id, user_id):
    ...
    score = sum(1 for response in responses if response['user_answer'] == response['correct_answer'])  # Correct answers
...
def student_academic_details():
    ...
    # Quiz details
    quizdetails = [row_to_dict(row) for row in db.session.execute(text("""
    ...
    # Subject counts
    subjectcount = [row_to_dict(row) for row in db.session.execute(text("""
    ...
    # Subject averages
    subjectmarks = [row_to_dict(row) for row in db.session.execute(text("""
...
def admin_search():
    ...
    if search_type == 'users':
        ...
        results = Questions.query.filter(
            Questions.question.ilike(f'%{search_query}%')
        ).all()
